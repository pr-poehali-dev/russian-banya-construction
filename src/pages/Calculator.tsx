    ...
    try {
      ...
      if (totalSize > 8) {
        setValidationMessage(
          `📦 Общий размер файлов слишком большой (${totalSize.toFixed(1)} МБ)\n\n` +
          `Максимум: 8 МБ всего\n\n` +
          `💡 Что сделать:\n` +
          `• Удалите некоторые файлы\n` +
          `• Или отправьте их частями в комментарии к заказу`
        );
        setShowValidationDialog(true);
        setIsSending(false);
        return;
      ...
      // Проверяем, есть ли в ошибке информация о размере файла
      const errorMessage = error instanceof Error ? error.message : String(error);
      
      if (errorMessage.includes('слишком большой') || errorMessage.includes('МБ')) {
        // Ошибка размера файла - показываем через диалог
        setValidationMessage(errorMessage);
        setShowValidationDialog(true);
      } else {
        // Другая ошибка - общее сообщение
        setValidationMessage('Ошибка отправки сметы. Проверьте интернет-соединение и попробуйте еще раз.\n\nЕсли ошибка повторяется, попробуйте уменьшить размер прикреплённых файлов.');
        setShowValidationDialog(true);
      }
    } finally {
      setIsSending(false);
                      ...
                      {attachedFiles.map((file, index) => (
                        <div key={index} className="flex items-center justify-between bg-emerald-50 p-2 rounded-lg">
                          <div className="flex items-center gap-2 flex-1 min-w-0">
                            <Icon name="FileText" size={16} className="text-emerald-600 flex-shrink-0" />
